...
Use meaningful, convention-following labels. When building from scratch, add all nodes first, then the edges between them. Colors are customizable via '#rrggbb' hex properties (set to null to restore the default): a node accepts "color" (border/accent), "fillColor" (interior) and "textColor" (label); an edge accepts "color" (its stroke). Set them through add_node/add_edge properties or update_node/update_edge changes. Space nodes out: increment X by ~200 per column and Y by ~150 per row so the layout is readable. Pick node sub-types from the lists below (they match the tool's accepted values).
