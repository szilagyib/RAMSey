...
function SidebarItemIcon({
  ...
  if (diagramType === 'fault_tree') {
    ...
    if (item.type === 'undeveloped_event')
      return (
        <svg {...svgProps}>
          <polygon
  ...
  if (diagramType === 'reliability_block_diagram') {
    ...
    if (item.type === 'input_terminal' || item.type === 'output_terminal')
      return (
        <svg {...svgProps}>
  ...
  if (diagramType === 'bow_tie') {
    ...
    if (item.type === 'threat' || item.type === 'consequence')
      return (
        <svg {...svgProps}>
          <rect
            x={6}
    ...
    if (item.type === 'preventive_barrier' || item.type === 'mitigative_barrier')
      return (
        <svg {...svgProps}>
          <rect
            x={22}
