...
- **`JWT_SECRET`**: rotating it invalidates *every* session immediately (all
...
- **`ANTHROPIC_API_KEY` / SMTP / Sentry DSN**: replace the env value and
  restart — nothing persisted depends on them.
- **Postgres password**: rotate in the DB, update `DATABASE_URL`
  (and `POSTGRES_PASSWORD` in `docker/.env.prod`), restart backend + worker.
...
- Minimum: nightly `pg_dump` of the `ramsey` database, retained per your
  policy; verify restore periodically (`pg_restore` into a scratch DB, run
...
|---|---|---|
| `npx vitest run` | engine + backend unit/API + frontend unit (265 tests) | DB only for the integration project |
